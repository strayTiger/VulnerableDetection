...
用法示例（推荐基线）：
python cwe_codebert.py --data-root "E:\codeBert" --model codebert_model --strip-comments --strip-strings --mask-sard-hints --epochs 3
python cwe_codebert.py --data-root "E:\codeBert" --model codebert_model --strip-comments --strip-strings --mask-all-identifiers --epochs 3
"""
...
# -------------------- 命令行参数 --------------------
def get_args():
    ap = argparse.ArgumentParser(description="CodeBERT CWE 漏洞检测训练/评估")
    ...
    ap.add_argument("--model", default="microsoft/codebert-base",
                    help="HuggingFace 模型名或本地 CodeBERT 权重路径")
    ...
    # 代码清洗（防泄露）
    # --strip-comments：如果命令行出现该选项，则把 C/C++ 注释（/*...*/ 与 //...）从源码中移除
    ap.add_argument("--strip-comments", action="store_true",
                    help="移除 C/C++ 注释（/*...*/ 与 //...），降低注释提示造成的泄露")
    # --strip-strings：如果命令行出现该选项，则把字符串/字符字面量替换为占位符（如 \"STR\"）
    ap.add_argument("--strip-strings", action="store_true",
                    help="把字符串/字符字面量统一替换为占位符 \"STR\"（字符也替为 'C' 的形态）")
    # --mask-sard-hints：掩蔽 SARD 样例中常见提示性标识符（good/bad/G2B/B2G/Sink/Source 等）
    ...
    # --mask-all-identifiers：大锤，所有形如标识符的词全部替换为 ID（用于泄露自检，不建议常开）
...
# -------------------- 模型：CodeBERT + 掩码均值池化 --------------------
class CodeBERTVD(nn.Module):
    """
    使用 CodeBERT（RoBERTa 架构）提取最后一层隐向量，
    对 padding 做掩码均值，再接线性层输出单一 logit（二分类）。
    """
    def __init__(self, model_name="microsoft/codebert-base", random_init=False):
        super().__init__()
        if random_init:
...
def train_loop(model, args, train_loader, val_loader, device, result_dir):
    ...
    model_save_path = os.path.join(result_dir, datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '_codebert.bin')
...
def main():
    ...
    model = CodeBERTVD(model_name=args.model, random_init=args.random_init).to(device)
    print(f"[Info] CodeBERT loaded from: {args.model}")
...
0   Overall  0.904186   0.844482  0.990844  0.911827  3058
1    CWE-74  0.871875   0.796020  1.000000  0.886427   320
2   CWE-190  0.943750   0.898876  1.000000  0.946746   320
3   CWE-191  0.853125   0.772947  1.000000  0.871935   320
4   CWE-369  0.946875   0.908571  0.993750  0.949254   320
5   CWE-400  0.890625   0.857143  0.937500  0.895522   320
6   CWE-404  0.787500   0.701754  1.000000  0.824742   320
7   CWE-573  0.971875   0.946746  1.000000  0.972644   320
8   CWE-665  0.940000   0.902439  0.986667  0.942675   300
9   CWE-670  0.924242   0.868421  1.000000  0.929577   198
10  CWE-704  0.921875   0.868852  0.993750  0.927114   320

--mask-sard-identifiers --epochs 1
Per-CWE metrics saved -> ./result\2025-09-25_14-37-06_perCWE_metrics.csv
        CWE  Accuracy  Precision    Recall        F1     N
0   Overall  0.979398   0.964512  0.995422  0.979723  3058
1    CWE-74  0.928125   0.874317  1.000000  0.932945   320
2   CWE-190  0.984375   0.975460  0.993750  0.984520   320
3   CWE-191  0.978125   0.963636  0.993750  0.978462   320
4   CWE-369  0.987500   0.981481  0.993750  0.987578   320
5   CWE-400  0.965625   0.940828  0.993750  0.966565   320
6   CWE-404  0.971875   0.957576  0.987500  0.972308   320
7   CWE-573  1.000000   1.000000  1.000000  1.000000   320
8   CWE-665  0.996667   0.993377  1.000000  0.996678   300
9   CWE-670  1.000000   1.000000  1.000000  1.000000   198
10  CWE-704  0.990625   0.987578  0.993750  0.990654   320
Figure saved -> ./result\2025-09-25_14-37-06_perCWE_4panels.png


distilcodebert   --mask-sard-identifiers --epochs 1
Per-CWE metrics saved -> ./result\2025-09-26_12-50-11_perCWE_metrics.csv
        CWE  Accuracy  Precision    Recall        F1     N
0   Overall  0.977763   0.967969  0.988228  0.977994  3058
1    CWE-74  0.928125   0.878453  0.993750  0.932551   320
...
5   CWE-400  0.993750   1.000000  0.987500  0.993711   320
6   CWE-404  0.953125   0.955975  0.950000  0.952978   320
7   CWE-573  0.993750   0.987654  1.000000  0.993789   320
8   CWE-665  0.986667   0.993243  0.980000  0.986577   300
9   CWE-670  1.000000   1.000000  1.000000  1.000000   198
10  CWE-704  0.965625   0.951515  0.981250  0.966154   320
Figure saved -> ./result\2025-09-26_12-50-11_perCWE_4panels.png
"""
