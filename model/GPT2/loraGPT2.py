...
# 如你有自定义 DataLoader.py（SplitCharacters / ListToCSV），这里兼容导入
try:
...
# ===================== 基础配置 =====================
MODEL_NAME      = "./gpt2_model"   # 如有本地权重可改这里
MAX_LEN         = 512               # GPT-2 支持到 1024；显存紧张可用 384/320/256
BATCH_SIZE      = 12
NUM_EPOCHS      = 10
LR              = 2e-5
WEIGHT_DECAY    = 0.01
...
LORA_ALPHA      = 32
LORA_DROPOUT    = 0.1
# GPT-2 注意力/MLP 典型命名："attn.c_attn", "attn.c_proj", "mlp.c_fc", "mlp.c_proj"
# 先用 ["c_attn","c_proj"]；更省显存可仅 ["c_attn"]；更强可再加 ["c_fc"]
LORA_TARGETS    = ["c_attn", "c_proj", "c_fc"]
MODULES_TO_SAVE = ["score"]         # ★ GPT2ForSequenceClassification 的分类头
...
# ===================== 数据读取与编码 =====================
def generate_id_label_text(json_path, use_split=True):
    """
    期望每行形如：
    {"func_name": "...", "func": "文本/代码", "target": 0/1}
    字段名不同请在此改写。
    """
    ids, labels, texts = [], [], []
...
def main():
    ...
    # 读取数据
    train_ids, train_y, train_text = generate_id_label_text(train_json, use_split=True)
    valid_ids, valid_y, valid_text = generate_id_label_text(valid_json, use_split=True)
    test_ids,  test_y,  test_text  = generate_id_label_text(test_json,  use_split=True)
    ...
    # Tokenizer & 编码（GPT-2 需要处理 pad）
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, use_fast=True)
    # GPT-2 默认无 pad_token：添加或把 pad 设为 eos
    if tokenizer.pad_token is None:
        tokenizer.add_special_tokens({"pad_token": "<|pad|>"})
    tokenizer.padding_side = "left"   # Causal LM 常用左填充，更稳
    ...
    train_masks   = train_masks_np.long()
    ...
    # 加载 base，并按需启用梯度检查点（注意 GPT-2 需要处理 pad token id）
    base = AutoModelForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=2)
    # 如果上面为 tokenizer 添加了 pad，需要 resize embedding
    if base.get_input_embeddings().num_embeddings != len(tokenizer):
        base.resize_token_embeddings(len(tokenizer))
    # 设置 pad_token_id（否则可能报错或产生不期望的损失计算）
    ...
    # Loss 曲线
    df_stats = training_stats if len(training_stats) else [{"epoch":0,"train_loss":0,"val_loss":0}]
    plot_loss_curve(df_stats, RESULT_DIR)
    ...
    # 结果保存
    out_csv = Path(RESULT_DIR, f"GPT2_LoRA_{datetime.datetime.now():%Y-%m-%d_%H-%M-%S}.csv")
