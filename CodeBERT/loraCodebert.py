...
from DataLoader import SplitCharacters, ListToCSV  # 你已有的工具
import matplotlib
matplotlib.use("Agg")
...
# ===================== 基础配置 =====================
MODEL_NAME      = "./codebert_model"   # 或 "microsoft/codebert-base"
MAX_LEN         = 512                  # 显存紧张可降到 384/320/256
BATCH_SIZE      = 12
NUM_EPOCHS      = 10
LR              = 2e-5
WEIGHT_DECAY    = 0.01
...
# ===== LoRA 配置（省显存的默认）=====
LORA_RANK       = 8          # 也可用 4（更省显存），或 2（更省）
LORA_ALPHA      = 32
LORA_DROPOUT    = 0.1
LORA_TARGETS    = ["query", "value", "key"]           # 需要更强再加 "key"；不建议匹配所有 "dense"
MODULES_TO_SAVE = ["classifier"]               # 关键：保存分类头，便于“最佳”完整恢复
...
def main():
    ...
    # 结果保存
    out_csv = Path(RESULT_DIR, f"CodeBERT_{datetime.datetime.now():%Y-%m-%d_%H-%M-%S}.csv")
